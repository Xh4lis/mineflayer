...
async function chercherBloc(nomDuBloc, minBlocks = 1, tentative = 1, astar, controller) {
  ...
  if (tentative <= 10) {
    ...
    const targetIds = bot.registry.blocksArray.filter(block => block.name.includes(nomDuBloc.toLowerCase())).map(block => block.id);
    ...
    if (blocsTrouves.length >= minBlocks) {
      ...
      // ASTAR
      const path = astar.findPath(Math.floor(maPosition.x), Math.floor(maPosition.y), Math.floor(maPosition.z), cible.x, cible.y, cible.z);
      if (path) {
          ...
          if (estArrive) {
              ...
              bot.chat("Bloqué en route, New A*");
...
async function goForWood(minBlocks = 10, tentative = 1, astar, controller) {
  ...
  if (tentative <= 10) {
    ...
    // SI IL NE TROUVE PAS (EXPLORATION)
    else {
      console.log("Pas assez de blocs ici, j'avance de 256 blocs pour explorer.");    
