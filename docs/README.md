...
- une fonction ```estPraticable(pos)``` qui détermine si le bot peut aller sur le bloc (donc (x,y,z)= air, (x,y+1,z)= air, (x,y-1,z)= solide)
- une fonction ```estCassableLatéral(pos)``` qui détermine si le bot peut casser le bloc à côté de lui (dans la direction donnée) et le bloc en dessous pour s'y frayer un chemin (checker si le bloc encore au dessus n'est pas de la lave/eau/gravier/sable/enclume ? | et que le bloc encore en dessous est solide)
